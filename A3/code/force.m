 function F = force(grd, ptcls, grd_to_ptcl, r_cut, sigma, epsilon, epsilon0)
  ...
  f = @(r_modulo,r, qq) -(24*epsilon*((1-2*((sigma./r_modulo).^6)).*((sigma./r_modulo).^6)./(r_modulo.^2))).*r + ...
      qq.*(1./(r_modulo.^3) + 3./(r_modulo.*r_cutQuadro) - 4./(r_cut^3)).*r./epsilon0;
  ...
  for ic = nonempty(:)'
    ...
    qq = ptcls.q(particle_index(:,1)).*ptcls.q(particle_index(:,2));
    ForzeCoppie(:,sub2ind([Nparticelle, Nparticelle],particle_index(:,1),particle_index(:,2))) = f(r_modulo,r,qq);
  end
